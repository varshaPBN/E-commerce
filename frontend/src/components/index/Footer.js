  ...
  return (
    <Box sx={{ backgroundColor: '#FDF8F2', borderTop: '1px solid rgba(59, 42, 26, 0.1)', py: 4 }}>
      <Container maxWidth="lg">
        ...
        >
          <Typography sx={{ fontSize: 12, color: '#666' }}>© 2025 Artloom Inc. All rights reserved.</Typography>
          <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, alignItems: { xs: 'flex-start', sm: 'center' }, gap: 2 , ml: 'auto'}}>
            <Typography sx={{ fontSize: 12, fontWeight: 600, color: '#3B2A1A', textTransform: 'uppercase' }}>Contact Us</Typography>
          ...
          <Box sx={{ display: 'flex', gap: 2 }}>
            {socialIcons.map(({ icon: Icon }, index) => {
              ...
              return (
                <IconButton key={index} size="small" sx={{ color: '#3B2A1A', '&:hover': { backgroundColor: 'rgba(59, 42, 26, 0.1)' } }}>
                  <Icon fontSize="small" />
                </IconButton>
              );
            })}
